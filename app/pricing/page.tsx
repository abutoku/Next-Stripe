...
const getAllPlans = async (): Promise<Plan[]> => {
  ...
  const { data: plansList }: any = await stripe.plans.list();

  const plans = await Promise.all(plansList.map(async (plan: any) => {
...
const PricingPage = async () => {
  ...
  const showSubscribeButton = !!user.session && profile && !profile.is_subscribed;
  const showCreateAccountButton = !user.session;
  const showManageSubscriptionButton = !!user.session && profile && profile.is_subscribed;
